...
| Current dependency baseline | Core/Plugin and sample Theme/Web `1.0.0-preview.20260915.1`, source commit `772e9ffbc05761a14f2a3a93700e4fef2b6c13b3`; OG-Q-005 resolved |
...
**Current release-matched API record (engine update, 2026-09-15):** a forced, no-cache solution restore resolves Core/Plugin/Theme/Web `1.0.0-preview.20260915.1`; installed NuGet metadata identifies commit `772e9ffbc05761a14f2a3a93700e4fef2b6c13b3`. The [generated-tag context contract](https://github.com/getscissorhands/ScissorHands.NET/blob/772e9ffbc05761a14f2a3a93700e4fef2b6c13b3/docs/website-documentation.md#generated-tag-route-context) and [generator source](https://github.com/getscissorhands/ScissorHands.NET/blob/772e9ffbc05761a14f2a3a93700e4fef2b6c13b3/src/ScissorHands.Web/Generators/StaticSiteGenerator.cs) show that route-only page documents now reach Razor before rendering. [PluginComponentBase](https://github.com/getscissorhands/ScissorHands.NET/blob/772e9ffbc05761a14f2a3a93700e4fef2b6c13b3/src/ScissorHands.Plugin/PluginComponentBase.cs), [CascadingMainLayoutBase](https://github.com/getscissorhands/ScissorHands.NET/blob/772e9ffbc05761a14f2a3a93700e4fef2b6c13b3/src/ScissorHands.Theme/CascadingMainLayoutBase.razor) and [ContentUrlHelper](https://github.com/getscissorhands/ScissorHands.NET/blob/772e9ffbc05761a14f2a3a93700e4fef2b6c13b3/src/ScissorHands.Core/Urls/ContentUrlHelper.cs) retain their public signatures. The sample already forwards `Document`; only the local URL helper needs to preserve the engine's pre-escaped synthetic route. Central major floats remain unchanged.

**Historical source verification (initial release):** the fetched `PluginComponentBase` source confirmed that `OnParametersSet` calls its base, validates the component `Id`, clears `Plugin`, validates configured IDs and duplicates with exact ordinal identity, then selects the matching manifest. The [PluginManifest source at that commit](https://github.com/getscissorhands/ScissorHands.NET/blob/7b5db6e1f27327cd8be50c08e4163e72e0a28425/src/ScissorHands.Core/Manifests/PluginManifest.cs) confirmed a shallow input copy into an ordinal `Dictionary` wrapped in `ReadOnlyDictionary`. This established the lifecycle/identity and option-snapshot contracts used here; it did not claim deep immutability of nested values.
...
For a generated `ContentKind.Page` with an empty/whitespace `SourcePath`, treat its slug as an engine-resolved route: validate percent escapes, then retain them after Core's normalization and literal-dot validation. The helper removes only the extra percent-sign escaping introduced by Core; it never decodes `%2F` into a path boundary or reconstructs a tag route. For example, `tags/c%23` stays `tags/c%23`, and `tags/literal%252F` stays `tags/literal%252F`. Source-backed pages and all posts keep the existing raw-slug escaping semantics, including escaping literal percent characters. Malformed generated-route escapes fail with field context and no payload disclosure. Route-only documents still use site-level metadata and never acquire a post creator.
...
| --- | --- | --- |
| P-FR-003 | OG-TR-001/003 | Shared metadata parity verified with both supplied and actual generated contexts; OG-Q-005 resolved for the recorded release |
| P-FR-005 | T-005 | Required-origin/image rules and generated-route escape preservation verified; OG-Q-005 closure includes exact tag canonical URLs |
...
At the initial package baseline, ordinary-page/equivalent-context parity passed and the tag-page discrepancy was retained as OG-Q-005. Those earlier results are historical; the following upgrade closes that gap. Production deployment/mounting, provider/crawler acceptance and actual publishing remain outside the recorded local evidence.

**Engine-update validation:** the [current shared upgrade record](../../TRD.md#engine-upgrade-evidence-2026-09-15) verifies the new release. The Open Graph suite now passes **201 tests**, including source-less route preservation, malformed escape rejection, unchanged source-backed/raw-slug semantics and context refresh. Nine newly added cases failed before the consumer escaping fix and pass afterward. Actual generator/renderer/plugin-runner tests and isolated sample runs additionally verify correct URLs for tag lists, ordinary/escaped/Unicode tag names, root/subpath deployments and preview/build modes without changing metadata eligibility.

## Gaps and readiness

The [PRD decision records](PRD.md#plugin-questions-and-acceptance-limits) retain product choices and routing IDs. OG-Q-001's insertion contract, OG-Q-002's image/output handling, OG-Q-003's parity/context/cancellation regressions and OG-Q-004's preview behavior remain implemented and verified. OG-Q-005 is resolved against the current engine release and consumer helper; older-host behavior remains documented below. No accepted plugin-runtime behavior is deferred. @justinyoo owns subsequent compatibility validation, actual publishing verification and release authorization.

### OG-Q-005: Generated tag pages receive unequal host context

**Status: resolved on 2026-09-15 against `1.0.0-preview.20260915.1`.** Upstream [issue #95](https://github.com/getscissorhands/ScissorHands.NET/issues/95) was fixed by [PR #96](https://github.com/getscissorhands/ScissorHands.NET/pull/96) and included in the new release. The original finding is retained for older hosts and traceability.

**Original integration finding (2026-09-15, UTC+09:00):** isolated generation with ScissorHands.Web `1.0.0-preview.20260914.1` exposed different tag-page `og:url` values. The [earlier generator source](https://github.com/getscissorhands/ScissorHands.NET/blob/7b5db6e1f27327cd8be50c08e4163e72e0a28425/src/ScissorHands.Web/Generators/StaticSiteGenerator.cs) explains that difference:
...
Consequently, at that release components received no document and applied the site-root fallback, while hooks retained the actual tag route. Titles, creator suppression and images matched, but `og:url` did not; the inputs were not equivalent.

**Closure:** the new engine provides an empty-source `ContentKind.Page` with the resolved slug before tag rendering. Its rendering metadata is route-only, so layout titles/descriptions/locales retain their site fallbacks. Hooks retain their synthetic titles and rendered HTML but receive the same slug. The existing sample cascade forwards this document unchanged. Open Graph preserves validated escapes in generated page slugs rather than escaping them twice; raw source-backed slugs remain unchanged. No tag label parsing, route inference, API invention or navigation changes are introduced.

**Evidence / upgrade:** [SampleTagRouteTests](../../test/ScissorHands.Plugins.Sample.Tests/SampleTagRouteTests.cs) exercises the actual released generator, renderer, layout and plugin runner in both modes with root/subpath and preview/build cases, checking every output URL against explicit expected routes. Names include `C# / <Tools>`, percent characters, a literal percent-encoded-looking name and Unicode. Source-less generated pages keep site metadata and omit creators; existing index/post/page/404 output remains covered. Isolated sample CLI generation independently verifies every tag URL, not just equality between two potentially wrong values. Refresh cached floating dependencies with `dotnet restore --force-evaluate --no-cache` and rebuild. Older `1.0.0-preview.20260914.1` hosts still require hook mode; custom layouts on the fixed release must forward `Document`. The closure is scoped compatibility evidence, not provider acceptance or a changed sign-off.
...
**Readiness:** implementation delivered against plugin PRD v0.7 and shared v0.8, now including verified generated-tag parity and OG-Q-005 closure on `1.0.0-preview.20260915.1`. Compatibility claims are limited to the recorded graph and integration scope. Actual publishing verification and release authorization remain required gates, not approved deferrals. Shared T-008 records setup and independent-verification limits; local validation is not production/crawler acceptance, a completed audit or release approval.

...
| P-FR-007 | Plugin authors can preview local changes without publishing packages | The user-requested [sample](sample/README.md) references local plugins and a NuGet.org engine, supports preview/build modes, and demonstrates component and paired-marker integration. It reuses packaged default-theme styling/scripts and the color toggle, without replacing the plugin insertion layout. Default configuration enables Open Graph only; analytics requires opt-in. Generated output is ignored and the sample is not packable. Local output/HTTP checks are not provider or production-host acceptance |
